...
class Trade(object):
    ...
    def in_trade(self):
        ...
        if float(self.money)<5:
            return None
        count=float(self.money)/(float(self.new_price)*18)
        id1=self.api.buy(self.name,float(self.buy1*0.99),count)
        p=float(self.buy1*0.99)
        print('欲买价%s'%p)
        time.sleep(60)
        count=float(self.money)/(float(self.new_price)*18)
        id2=self.api.buy(self.name,float(self.buy1)*0.997,count)
        print('欲买价%s'%float(self.buy1*0.997))
        time.sleep(30)
        self.api.cancel(self.name,id1)
        time.sleep(1)
        self.api.cancel(self.name,id2)
        time.sleep(2)
    ...
    def out_trade(self):
        print('准备卖出')
        count=self.on_hand
        id=self.api.sell(self.name,float(self.sell1*1.04),count)
        time.sleep(60)
        self.on_hand = self.api.get_user_info(self.name)
...
def start_all(api):
    aims=['xrp_usdt','eos_usdt','bch_usdt','iota_usdt']
    for i in aims:
        t= Trade(i, api)
